...
local label     = require 'fs50_subset_classes'
-- print(label)
...
local train_files = require 'fs50_subset_train_images'
local test_files = require 'fs50_subset_test_images'
local train_labels = require 'fs50_subset_train_labels'
local test_labels = require 'fs50_subset_test_labels'
...
-----------------------------------------------------------------------------------------
function calculateKernelDistance()
  result = torch.Tensor((trsize - current_pool_size))
  unlabeled_training_indices = torch.Tensor((trsize - current_pool_size))

  local labeled_features = torch.Tensor(current_pool_size, 4096)
  local unlabeled_features = torch.Tensor((trsize - current_pool_size), 4096)
  local labeled_counter, unlabeled_counter, input, prediction, current_output
  labeled_counter = 0
  unlabeled_counter = 0
  print('[Computing features of labeled and unlabeled pool samples...]')
  for i = 1,trsize do
    xlua.progress(i, trsize)
    ...
    if untrained_training_images[i] then
      ...
      unlabeled_features[unlabeled_counter] = current_output:clone():float()
      unlabeled_training_indices[unlabeled_counter] = i
    else
      labeled_counter = labeled_counter + 1
      labeled_features[labeled_counter] = current_output:clone():float()
    end
  end

  print('[Calculating distance between each unlabeled sample and all the labeled samples...]')
  for i = 1,(trsize - current_pool_size) do
    xlua.progress(i, (trsize - current_pool_size))
    local distK_Xu_Yl = torch.Tensor(current_pool_size)
    for j = 1,current_pool_size do
      distK_Xu_Yl[j] = torch.dist(unlabeled_features[i], labeled_features[j])
    end
    result[i] = torch.min(distK_Xu_Yl)
  end
...
function train()
   ...
   local correct_predictions = 0
    ...
    if active_learning_flag then
      ...
      kernel_distance_measures, unlabeled_training_indices = calculateKernelDistance()
      samples_added_per_update = math.min(samples_added_per_update,(training_threshold - current_pool_size))
      top_k_values, top_k_indices = kernel_distance_measures:topk(samples_added_per_update, true)
...
while true do
  if initial_training_counter > 0 then
    ...
    --  epoch_per_update = 5
    --end
    print('[training using kernel farthest first method...]')
    train()
    print('[testing using kernel farthest first method...]')
    test()
  end
end
--net:evaluate()
...
--sample_result, sample_indices = calculateKernelDistance()
--print(sample_result)
-----------------------------------------------------------------------------------------------------------------------
