...
explored_samples_added_per_update = 20
exploited_samples_added_per_update = 20
optimization_factor = 0.25
training_threshold = optimization_factor * trsize
...
opt.save = opt.save .. '_testing_' .. dataset_code .. '_' .. experiment_code .. '_c' .. total_classes .. '_bs' .. opt.batchSize .. '_lr' .. opt.learningRate .. '_lrd' .. opt.learningRateDecay .. '_wd' .. opt.weightDecay .. '_m' .. opt.momentum .. '_' .. os.date("%y%m%d%H%M%S")
print(opt)
---------------------------------------------------------------------------------------------------------
...
function calculateExpectedGradientLength(sample)
  ...
  for counter = 1,egl_top_k_count do
    ...
    --current_parameters,current_grad_parameters = current_module:parameters()
    --current_grad_weights = current_grad_parameters[1]
    --print('[calculated gradWeights...]')
    --tensor_grad_weights = torch.Tensor(current_grad_weights)
    --current_grad_biases = current_grad_parameters[2]
    --print('[calculated gradBiases...]')
    --tensor_grad_biases = torch.Tensor(current_grad_biases)

    --local grad_length_square = 0
    --for i = 1,total_classes do
    --  grad_length_square = grad_length_square + torch.pow(tensor_grad_biases[i], 2)
    --  for j = 1,4096 do
    --    grad_length_square = grad_length_square + torch.pow(tensor_grad_weights[i][j], 2)
    --  end
    --end
    --grad_length = torch.sqrt(grad_length_square)
    --current_measure = current_measure + math.exp(prediction[label])*grad_length
    --print(final_layer_grad_parameters:dim())
    --current_measure = current_measure + math.exp(prediction[label])*torch.norm(current_grad_parameters)
...
function train()
   ...
   local correct_predictions = 0
    ...
    --         local current_measure = calculateExpectedGradientLength(trainData.data[j])
    ...
    if active_learning_flag then
      ...
      print('[Exploring...]')
      local EGL_measures = torch.Tensor(trsize)
      for i = 1,trsize do
        xlua.progress(i, trsize)
      ...
      top_k_values, top_k_indices = EGL_measures:topk(explored_samples_added_per_update, true)
      for i = 1,explored_samples_added_per_update do
      ...
      print('[Exploiting...]')
      local confidence_measures = torch.Tensor(trsize)
      for i = 1,trsize do
        xlua.progress(i, trsize)
        if untrained_training_images[i] then
      ...
      top_k_values, top_k_indices = confidence_measures:topk(exploited_samples_added_per_update, true)
      for i = 1,exploited_samples_added_per_update do
        if untrained_training_images[top_k_indices[i]] then
          table.insert(labeled_pool_samples, trainData.data[top_k_indices[i]])
          table.insert(labeled_pool_labels, trainData.labels[top_k_indices[i]])
          current_pool_size = current_pool_size + 1
...
--end
--current_measure = torch.Tensor(400)
--for i = 1,400 do
--  xlua.progress(i, 400)
--  current_measure[i] = calculateExpectedGradientLength(trainData.data[i])
--end
--top_values, top_indices = current_measure:topk(40, true)
--print(top_values)
--print(top_indices)
-----------------------------------------------------------------------------------------------------------------------
